...
To this end, most properties in this proposal's `AbstractControl` API used to store a default value. Calling `reset()` used those default values to reset the form control to its user specified default state. After using the API in this proposal for a bit though, I decided that `reset()` didn't make sense in practive.

If you wish to voice your opinion, you can comment in the (now closed [#4](https://github.com/thefliik/reactive-forms-2-proposal/issues/4)).

**The arguments against `reset()`:**

#### Usage with `ControlAccessor` API

An obvious place I could see myself wanting to use `reset()` would be in the context of the `ControlAccessor` API this repo uses. If I created a `ControlContainerAccessor` which used a `FormGroup` to represent a person form, I might like to supply default values to that form with the ability to `reset()` those values at the push of a button. This would be nice, as it would encapsulate the default values in the `ControlContainerAccessor` component.

Unfortunately this will not work as expected, because the ControlContainerAccessor's `control` property, when linked to an outside `FormGroup` using a `FormGroupDirective`, will be reset so that it has the same state as the outside `FormGroup` using `replayState({includeDefaults: true})`. This will make the ControlContainerAccessor's defaults equal to the provided FormGroup's defaults, meaning the behavior of `reset()` cannot be counted on by the `ControlContainerAccessor`.

Instead, to accomplish the goal of encapsulating the reset behavior of the `ControlContainerAccessor`, you could manually reset individual properties (in the same way that you can using the current `ReactiveFormsModule`). But obviously this doesn't involve usage of the `reset()` method.

Another possibility is for FormControlDirective's to not sync state using `replayState({includeDefaults: true})` on init, but to instead use `replayState()`. This would allow the `ControlContainerAccessor` to have different default values than a linked form group. Unfortunately, if the end user manually changes a default value of the linked `FormGroup`, that state change will still flow over to the `ControlContainerAccessor` at which point it will processes that change, updating the relevant default value. This means that the `ControlContainerAccessor` still cannot rely on the behavior of `reset()`.

#### Handling `value` as well as `valueDefault` state changes

Because properties also store a default value, and that default value is updating with a different type of state change, it can complicate manually processing state changes -- it certainly increases the mental model. For example, if you wish to link two form controls, but transform the values between them (e.g. convert one control's `string` value to a `Date` value in the other control) you need to remember to not only process `value` state changes, but the much less common `valueDefault` state change as well.

For example, here the user forgets about the possibility of the `valueDefault` state change:

```ts
const controlString = new FormControl('');
const controlDate = new FormControl<Date | null>(null);

controlString.changes.pipe(
  map(state => {
    if (state.type === 'value') {
      return {
        ...state,
    ...
    return state;
  }).subscribe(controlDate.source);

controlDate.changes.pipe(
  map(state => {
    if (state.type === 'value') {
      return {
        ...state,
    ...
    return state;
  }).subscribe(controlString.source);

controlString.stateChange({
  type: 'valueDefault',
  value: '',
});
```

When the '`valueDefault'` state change is emitted from `controlString`, it will be synced with the `controlDate` as is. Meaning that the default value for `controlDate` is now the string `""` (a type error), and calling `controlDate.reset()` will set the value to a string (another type error).

While this is the most severe case of forgetting about default values, most every property in this API proposal currently has them and a similar scenario could play out in other ways (e.g. using the `touched` and `touchedDefault` properties).

#### The difference between `replayState()` and `reset()`

The `replayState()` method returns an observable containing the current state of an abstract control. Subscribing a control's `source` to this observable will update that control with the new state. By default, it doesn't include the default values of the abstract control, but if you provide the `includeDefaults: true` option it will.

`replayState()` is similar to `reset()`, and can be used to accomplish the same goal. For instance, if, after initializing a new FormControl, you use `replayState()` to save a snapshot of the current state, you can completely reset that form control at any time by subscribing the form control's `source` to the saved `replayState()` observable.

- In fact, if I were creating a `ControlAccessor` component which I wanted to be able to reset using encapsulated data, I would turn to `replayState()`.

While `replayState({includeDefaults: true})` is a snapshot of _all_ the control's state, `reset()` might not reset all aspects of a control. Instead, `reset()` only effects those properties which are "resettable" (a subjective concept) using the available default values. At the moment, the two methods are highly analogous.
