...
export class NgCompatFormControlDirective implements OnChanges, OnDestroy {
  ...
  protected valueAccessor: NgControlValueAccessor | null;
    ...
    this.subscriptions.push(
      this.control.changes
        .pipe(
          filter(
            ({ type, meta }) =>
              (type === 'setValue' || type === 'patchValue') &&
              !(meta && meta[this.ngControl.id as any]),
          ),
          map(({ value }) => value),
        )
        .subscribe(value => {
      ...
      this.control.changes
        .pipe(
          filter(state => {
            ...
            if (state.sources.includes(this.ngControl.id)) {
              return false;
            }
            if (state.ngControl) {
              return false;
